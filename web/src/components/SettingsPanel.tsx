...
export default function SettingsPanel({
  ...
  const dangerClass = `${itemClass} text-danger hover:text-danger`

  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => { setOpen(o => !o) }}
        className="px-3 py-1 text-xs font-medium border rounded-md cursor-pointer leading-5 transition-colors bg-surface-inset text-fg-muted border-edge hover:bg-edge hover:text-fg"
        title="Settings"
      >
        <svg className="w-3.5 h-3.5 align-middle" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-48 bg-surface border border-edge rounded-md shadow-lg z-50 py-1 text-fg">
          ...
          <button
            ...
            disabled={!hasComments}
            onClick={() => { if (hasComments) { onClearComments(); setOpen(false); } }}
          >
            <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          <button
            ...
            onClick={() => { if (hasReviewed) { onClearReviewed(); setOpen(false); } }}
          >
            <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
            </svg>
            <span>Clear reviews</span>
          </button>
